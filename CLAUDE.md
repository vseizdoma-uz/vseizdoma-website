...
## Changelog
- **11.09.2026** **Провал подстраниц 19–29.08 закрыт сам + «старые вещи» в description главной.** С 30.08 подстраницы вернулись без наших действий и выше прежнего: 8.8 клика/день против 6.3 до провала (03–16.08), главная 21 против 18. Неделя 31.08–06.09 — рекорд за всю историю: 230 кликов (прошлый максимум 189). Растут страницы, правленные 12–13.08: `skupka-kovrov` 3→18, `kuplyu-vse-iz-doma` 5→16, `skupka-posudy` 10→17, `skupka-tehniki` 5→9. Просела только `kupit-mikrovolnovku` (5→0) — не разбиралась. **Кластер «старые вещи»** (~1200 показов за 4 недели) целиком идёт на главную на позициях 7–8; средний CTR кластера ~6%, но три главных запроса («старые вещи покупаем (ташкент)», «скупщики старых вещей») дают 17 кликов на 497 показов = 3.4%. Правка: «старые вещи» в description (+og/twitter), 158 символов, «Куплю все из дома» остался первым; «демонтаж» и «за» убраны ради длины. **Title не трогали** — на нём держится бренд (72% кликов), риск для бренда больше, чем выигрыш на кластере. Ожидание скромное: ~+10–15 кликов/мес, и только если Google возьмёт наш description, а не соберёт сниппет из текста (H3 «Где продать старые вещи» уже есть). Замер: 25.09 против 28.08–10.09, по трём запросам кластера.
- **13.08.2026 (2)** **Тема «скупка бытовой техники» + найден системный баг с H1.** Взялись за самую крупную дыру: «скупка бытовой техники (ташкент)» — 355 показов за 90 дней на позиции ~20, 2 клика; при этом мебельная тема при **том же объёме контента** (1503 против 1525 слов), той же структуре и том же числе входящих ссылок (34) держит позицию 9.3. Разница не в нас, а в выдаче: в технике конкурируют **профильные домены** — `kuplyu-tashkent.uz`, `skupkaboom.uz`, `skupka-tashkent.uz`, `skupka-texnika.uz` — плюс сеть Instagram-профилей с одним телефоном 907992244 (`@skupkabitovoy.tehniki.tashkent`, `@kuplyu_bitovaya_texnika`); в мебели такой плотности нет. ⚠️ **Браузерная выдача врёт:** в залогиненном Chrome мы показывались на 3-й позиции (Google даже подмешивает виджет Search Console), тогда как GSC даёт стабильные 16–28 за три месяца — верить только GSC. **СИСТЕМНЫЙ БАГ:** на 6 страницах `subpage.js` перезаписывал H1 значением `PAGE_TRANSLATIONS.heroTitle`, в котором **не было слова «дорого»** — то есть Googlebot после рендеринга видел H1 без CTR-модификатора, раскатанного в июне (`kupit-gazplitu`, `kupit-konditsioner`, `kupit-stiralnuyu`, `kupit-televizor`, `skupka-mebeli`, `skupka-tehniki`). Переводы приведены к HTML; сверены **все 159 заголовков** с `data-i18n` — расхождений больше нет. Отдельно выяснено, что отсутствующий ключ безопасен (`void 0!==n[t]` — JS не трогает текст), поэтому 8 непереведённых FAQ-ключей на ряде страниц последствий не дают. **`skupka-tehniki` (RU):** H1 → «Скупка бытовой техники б/у в Ташкенте дорого» (точное вхождение главного запроса), все H2 приведены к ключу «бытовая техника» синхронно в HTML и i18n, карточки 3→6 (кондиционеры отделены от ТВ; добавлены телевизоры, газовые/электроплиты, микроволновки+посудомойки) с контекстными ссылками на `kupit-televizor` и `kupit-gazplitu`, переводы ru/uz/en; «бытов» 11→18 вхождений, 1503→1591 слово. Сайтвайд: анкор «Скупка техники в Ташкенте» → «Скупка бытовой техники в Ташкенте» на 12 страницах. **UZ не трогали намеренно** — узбекский спрос по технике 22 показа за 180 дней и 0 кликов. Проверено: FAQ 14==schema 14, JSON-LD валиден. Запушено, IndexNow + Indexing API на 6 URL.
- **13.08.2026** **Замер GSC + добитый сниппет `kupit-shkaf` + диагноз `kupit-stiralnuyu`.** Замер за 15.07–12.08 против 17.06–14.07 (обе даты после 27.04, показы сравнимы): клики **351→581 (+65%)**, показы +4%, CTR 3.63%→5.78%, средняя позиция 7.27→7.60 (чуть хуже). Вывод: рост целиком из **CTR-части** пакета 28.07, не из ранжирования — «куплю все из дома ташкент» дал 9→32 клика при неизменной позиции 7.2→7.1. Рекорд 8 августа: 32 клика / 413 показов. UZ поехал после «eski»-фикса: `uz/skupka-mebeli` 3→11 кликов, `uz/kupit-mebel` 2→7. **`kupit-shkaf` (RU+UZ):** снят запрет на title/description — разворот 12.08 переписал контент, но сниппет и **вся schema** остались продажными (Service, BreadcrumbList, Product, HowTo «Как купить шкаф б/у» с шагами «пришлём фото моделей → привезём, соберём»). Всё переведено на скупку, шаги HowTo переписаны 1:1 с видимой секцией. Попутно узбекская орфография: `kupeler`→`kupelar`, `garderobler`→`garderoblar` (мн. ч. -lar), 2 файла. **`kupit-stiralnuyu` — разобрано:** просадка июля (631→305 показов, поз 8.0→10.1) **не техническая и не от наших правок** — индексация/canonical/rich results чистые (URL Inspection API), объём 1470 слов на уровне соседей, интент страницы согласован, а RU и EN версии просели синхронно, хотя обе не трогали. Настоящая причина структурная: **52% показов темы — покупательский интент** («куплю/купить/олх»), продавцовый только 22%; по главному запросу «куплю стиральную машину бу ташкент» (329 показов, 27% темы) в топе OLX ×3, Texnomart, BirBir, CA Store — скупочная страница там чужая (7 кликов на 329 показов). Конкуренты именно в скупке — соцсети: Instagram @kuplyu_mebel_v_tashkente (поз. 3, выше нас) и FB «Скупка стиральных машин в ташкенте» (поз. 8). Правка сделана минимальная: «продать» в title (ловит «продам стиральную машину бу», 92 показа поз 9.6), description «стиралку»→«стиральную машину», og:title синхронизирован (расходился с title), дата «Обновлено» 6 июня→13 августа (в сниппете Google висело 6 июня). **Вкладываться в товарные темы против OLX смысла нет** — стиралка даёт ≈5 кликов/мес, бренд «все из дома» — 72% всех кликов.
- **12.08.2026 (2)** **Разворот двух «продажных» страниц на скупку** — по решению владельца «пусть продают нам». Данные: `kupit-shkaf` дал 2 клика на 193 показа за 180 дней (CTR 1.0% против 5–6% по сайту) при хороших позициях 4.8–8.2; `kupit-mebel` — 11 кликов на 449 показов (2.4%), причём 312 из 449 показов приходили по скупочным запросам. Причина низкого CTR — рассогласование: title/description обещали магазин, H1 и hero говорили о скупке, секции были про покупку; UZ-версия шкафа вообще целиком про продажу, то есть RU и UZ разошлись по смыслу. **`kupit-shkaf` (RU+UZ) переведён на единый скупочный интент:** H1 и hero UZ переписаны, H2, карточки, шаги, 13 FAQ, alt-тексты, seo-блок и CTA — обе версии. **`kupit-mebel` интент НЕ менял** (столкнулся бы со `skupka-mebeli`) — вместо этого добавлена врезка «А если вы хотите не купить, а продать мебель?» с условиями и ссылками на `skupka-mebeli` и `kuplyu-vse-iz-doma`. **`kuplyu-vse-iz-doma`:** блок про Самарканд и регионы (порога нет, оценка по фото/видео, логистика по договорённости — со слов владельца) + 2 FAQ, итого 16. ⚠️ **title и description не трогал** по прямому запросу — сниппет `kupit-shkaf` остался продажным, это единственный нестыкующийся кусок. Регионы: 115 показов за 180 дней на 4 запросах, отдельную страницу решено не делать. Проверено: FAQ==schema, JSON-LD валиден, узбекская орфография чистая, число секций совпадает с прежним (после того как regex однажды съел CTA — восстановлено из git).
- **12.08.2026** **Брендовая `kuplyu-vse-iz-doma` (RU+UZ) углублена + закрыта тема переезда/гаража.** Была самая тонкая страница сайта — 842 слова и 8 FAQ против 1100–1800 у остальных, позиция 17.0 — при том что бренд даёт **72% всех кликов**. RU 842→1628 слов, UZ 844→1713, FAQ 8→14 обе, FAQPage schema пересобрана из HTML 1:1. Добавлены: секция «От чего зависит цена выкупа» (6 карточек), большой блок «Куплю всё из дома при переезде» (что забираем из гаража/подвала/дачи, освобождение квартиры целиком, **что НЕ покупаем**, сравнение с распродажей по частям), финальный seo-блок 3→6 абзацев с перелинковкой на 6 подстраниц. **Отдельную страницу `skupka-pri-pereezde` решено НЕ делать:** за 90 дней слово «переезд» дало 0 запросов, вся тема — один запрос «куплю все из дома и гаража ташкент» (98 показов, поз. 7.3), который уже обслуживает главная; новая страница разделила бы сигнал, как это видно на паре главная/`skupka-posudy`. Тема закрыта блоком внутри брендовой. Проверено: FAQ==schema, JSON-LD валиден, узбекская орфография чистая, даты и sitemap → 12.08. IndexNow + Google Indexing API.
- **12.08.2026** Проверена «каннибализация посуды» — её нет: по запросу «куплю все из дома ташкент посуда» главная берёт 74 показа (поз. 7.4), `skupka-posudy` — 4 показа (поз. 9.2), то есть 95% у главной, что корректно для брендового запроса; чистый «скупка посуды в ташкенте» идёт на подстраницу (поз. 8.8). Google разделил их правильно, вмешательство не требуется.
